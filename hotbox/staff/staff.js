...
function loadData(){
        var x = new Request({
                url: 'index.php?option=com_jumi&fileid=219&action=load_data',
                        method: 'get',
                        noCache: true,                        
...
function deleteRecord(staffno){
        var x = new Request({
                url: 'index.php?option=com_jumi&fileid=219&action=delete_record&staff_no2='+staffno,
                        method: 'get',
                        noCache: true,                        
                    onComplete: function(response){
                     //alert(response);
...
function updateExt(stafno,ext){
        var x = new Request({
                url: 'index.php?option=com_jumi&fileid=219&action=update_ext&staff_no='+stafno+'&ext_no='+ext,
                        method: 'get',
                        noCache: true,                        
                    onComplete: function(response){
                     //alert(response);
...
function listStaff() {
	...
	var x = new Request ({
		url: 'index.php?option=com_jumi&fileid=219&action=display_staffData',
			noCache: true,
			method: 'get',
			timeout: 5000,
			onTimeout: function() {
...
function list_users(data) {
	...
	var x = new Request({
		url: 'index.php?option=com_jumi&fileid=219&action=list_users&id='+id+'&scond='+scond,
		method: 'get',
		timeout: 10000,
		onTimeout: function() {
			x.cancel();
...
function editStaff(id) {
	...
	var x = new Request({
		url: 'index.php?option=com_jumi&fileid=219&action=edit_staff&id='+encodeURI(id),
			method: 'get',
			noCache: true,
			onComplete: function(response){
...
function saveStaff(frm,action) {    
        var x = new Request({
                url: 'index.php?option=com_jumi&fileid=219&action=save_edit_staff',
                        method: 'post',
                        noCache: true,
                        data: frm,
                    onComplete: function(response){
                    // alert(response);
                    if (parseInt(response) == 1){
                                alert('Card number updated successfully.');
                                window.location.reload();
                        }
