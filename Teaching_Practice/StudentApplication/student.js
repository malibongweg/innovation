    ...
    $('criteria-form').addEvent('submit', function(e){
       //var grade_desc = $('grade').get('value');
        //alert(response);
         e.stop();
         var x = new Request({
             //A request is sent to the fileid 178 which is the student_ajax.php file aand the action it will execute is the save_criteria that executes sql statements

             url: 'index.php?option=com_jumi&fileid=178&action=save_criteria',
             method: 'post',
             ...
             onComplete: function(response){
                 ...
                 if (response == "-1") {
                     ...
                     alert('Please make another selection already chosen');
                 }
                 else if (response=='-7') {
...
function displayInfo() {
	...
	var y = new Request({
		url: 'index.php?option=com_jumi&fileid=178&action=display_stud_info&uid='+$('login-name').value,
        noCache: true,
		method: 'get',
		timeout: 15000,
		onTimeout: function() {
...
{    
    var html = '<table width="100%" border="0" style="table-layout: fixed">';
    var y = new Request({
        url: 'index.php?option=com_jumi&fileid=178&action=show_subjects&stno='+stno,
        noCache: true,
        method: 'get',
        async: false,
        onComplete: function(response) {
...
function  listSubs(schoolname)
{
    ...
    //$('grade').empty();
    var y = new Request({
		url: 'index.php?option=com_jumi&fileid=178&action=displaysubs&sname='+schoolname,
        noCache: true,
        method: 'get',
        async: false,
        onComplete: function(response) {
...
function listGrades(subj)
{
    $('grade').empty();
    var y = new Request({
        url: 'index.php?option=com_jumi&fileid=178&action=display_grades&subj='+subj,
        noCache: true,
        method: 'get',
        async: false,
        onComplete: function(response) {
...
function deleteSubject(subj)
{
   ...
   //var stno = $('login-name').value;
    ...
    var x = new Request({
        url: 'index.php?option=com_jumi&fileid=178&sqno='+subj+'&stno='+$('login-name').value+'&school_name='+school_name+'&action=delete_subject',
...
function list_subs()
{
    ...
    var html = '<table width="100%" border="0" style="table-layout: fixed">';
    var x = new Request({
       url: 'index.php?option=com_jumi&fileid=178&stno='+$('login-name').value+'&action=list_subs',
        method: 'get',
        onComplete: function(response){
...
//    $('ajax').setStyle('display','block');
//	var y = new Request({
//		url: 'index.php?option=com_jumi&fileid=178&action=delete_subj&code='+subcode,
...
function displayInfo2() {
    ...
    // a request is sent through with the action display_stud_info and also checks the person that logged and and will do the request as per the uid
    var y = new Request({
        url: 'index.php?option=com_jumi&fileid=178&action=display_stud_info2&uid='+$('login-name').value,
        noCache: true,
        method: 'get',
        timeout: 15000,
        onTimeout: function() {
