...
window.addEvent('domready', function() {   
   ...
   $('new-school').addEvent('click', function(){
    ...
    $('school-info').addEvent('submit', function(e) {
        e.stop();
        var x = new Request({
            url: 'index.php?option=com_jumi&fileid=172&action=save_school',
            method: 'post',
...
{
    var x = new Request({
        url: 'index.php?option=com_jumi&fileid=172&sid='+school_id_delete+'&action=delete_school',
        method: 'get',
        onComplete: function(response) {
            if (response == 1)
            {
...
$('buttons').setStyle('display','block');
        var x = new Request({
            url: 'index.php?option=com_jumi&fileid=172&action=addschool&schoolname=' + schoolname + '&address1=' + address1 + '&address2=' + address2 + '&address3=' + address3 + '&address4=' + address4 + '&postcode=' + postcode + '&tel=' + tel + '&fax=' + fax + '&email=' + email + '&principal=' + principal + '&liaison=' + liaison + '&lang_used=' + lang_used + '&num_students=' + num_students + '&dt=' + new Date().getTime(),
            method: 'get',
            onComplete: function(response) {
                if (response == 1)
                {
...
function listSchools() {
    ...
    var x = new Request ({
        url: 'index.php?option=com_jumi&fileid=172&action=display_schools',
        noCache: true,
        method: 'get',
        onComplete: function(response) {
...
function srchSchool()
{
    ...
    var x = new Request({
        url: 'index.php?option=com_jumi&fileid=172&school=' + $('srch').value + '&action=srch&dt=' + new Date().getTime(),
        method: 'get',
        onComplete: function(response) {
            $('school-List').empty();
            var row = json_parse(response, function(data, text) {
...
function displaySchoolInfo(school_id) {
    ...
    $('buttons').setStyle('display','block');
    var x = new Request({
        url: 'index.php?option=com_jumi&fileid=172&school_id=' + school_id + '&action=info&dt=' + new Date().getTime(),
        method: 'get',
        onComplete: function(response) {
            $('school-List').empty();
            var row = json_parse(response, function(data, text) {
...
{
    var x = new Request({
        url: 'index.php?option=com_jumi&fileid=172&id='+id+'&sid='+sid+'&action=delete_subject',
        method: 'get',
        onComplete: function(response) {
...
function listSubjects(scode) {
    ...
    var x = new Request ({
        url: 'index.php?option=com_jumi&fileid=172&action=display_subjects&schoolcode='+scode,
        noCache: true,
        method: 'get',		
                onComplete: function(response) {
                    var row = json_parse(response,function(data,text) {
