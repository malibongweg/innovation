...
$array = $_GET['myArray'];
    ...
    if($_GET['action'] == 'allocate_student')
    {
        ...
        for($i = 0; $i < count($array); $i++) {
            ...
            if ($dbo->getNumRows() == 0) {
                if(strlen($array[$i]) == 9) {
                    $sql = sprintf("insert into teaching_practice.EvalStud(StudentNo, evalseqno, schoolcode) values('%s',%d, %d)", $array[$i], $row1->evalseqno, $scode);
                    $dbo->setQuery($sql);
                    $result = $dbo->query();
                }
