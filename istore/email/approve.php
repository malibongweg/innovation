...
if ($dbo->getNumRows() == 0) {
	...
	$sql = sprintf("select istore_order_status_id,store_id,client_uid,creator_uid,order_no,order_date,cost_centre,comments,istore.istore_order_status.`status`
		...
		<?php
				...
				$sql = sprintf("update istore.istore_orders set istore_order_status_id = 3 where order_no = '%s'",$order);
				$dbo->setQuery($sql);
				$dbo->query();
		?>
					<p >Order# <?php echo $order; ?> was APPROVED.<br /> Requestor and client will be notified of current order status.<br />
				...
				window.addEvent('domready',function(){
						$('show-emailing').setStyle('display','block');
							var x= new Request({
							url: '/index.php?option=com_jumi&fileid=157&action=sendOrderApproval&orderno=<?php echo $order; ?>',
							method: 'get',
							noCache: true,
