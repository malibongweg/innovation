...
<input type="hidden" id="jtype" value="<?php echo $_GET['jtype']; ?>" />
<input type="hidden" id="urgent" value="<?php echo $_GET['urgent']; ?>" />
...
function reportJobcards(){
	...
	var y = new Request({
		...
		onComplete: function(response){
			...
			switch (parseInt(jtype)){
				case 0: response = response + ' (All Jobcards)';
				break;
				case 1: response = response + ' (Open Jobcards)';
				break;
				case 2: response = response + ' (Closed Jobcards)';
			}
			if (parseInt(urgent) == 1) {
				response = response + ' [URGENT]';
			}
			doc.text(response,7,line);/*Set Campus Name*/
			line = line + 7;
			var pw = doc.getPageWidth();
			doc.setFontSize(10);
			...
			line = line + 4;
			doc.setFontSize(8);
			doc.text('CAPETURE DATE',7,line);
			doc.text('JOBCARD#',40,line);
			doc.text('ASSIGNED DATE',70,line);
			doc.text('BUILDING',100,line);
			doc.text('STATUS',160,line);
			doc.text('OPEN HOURS',180,line);
			doc.setFontStyle('normal');
			line = line + 5;
				var z = new Request({
					url: '/scripts/jobcards/chart_data.php?action=getOpenJobcards&campus='+campus+'&start-date='+sdate+'&end-date='+edate+'&jtype='+jtype+'&urgent='+urgent,
					method: 'get',
					noCache: true,
					async: false,
					onComplete: function(response){
						...
						doc.text(data2.Records[x].capture_date,7,line);
						doc.text(data2.Records[x].id,40,line);
						doc.text(data2.Records[x].assigned_date,70,line);
						doc.text(data2.Records[x].build_name,100,line);
						doc.text(data2.Records[x].status_desc,160,line);
						doc.text(data2.Records[x].openhours,180,line);
						line = line + 5;
						}
						if (line >=250) {
							drawPageNo();
						...
						doc.save('/scripts/jobcards/tmp/openJobcards.pdf');
