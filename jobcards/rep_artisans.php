...
foreach($_POST as $key=>$value){
	if ($key == 'start_date'){
		echo '<input type="hidden" id="start-date" value="'.$value.'" /><br />';
	}
	if ($key == 'end_date'){
		echo '<input type="hidden" id="end-date" value="'.$value.'" /><br />';
	}
	if (substr($key,0,7) == 'artisan'){
		$artisan .= $value.';';
	}
}
$artisan = substr($artisan,0,-1);
echo '<input type="hidden" id="artisans" value="'.$artisan.'" />';
?>
...
function setHeader() {
	...
	doc.setFontSize(10);
	doc.myText('Artisan Labour Report',{align: 'right'},1,15);
	doc.setFontSize(10);
...
setTimeout('reportLabour();',500);

function reportLabour(){
	setHeader();
	var sdate = $('start-date').get('value');
	var edate = $('end-date').get('value');
	var artisans = $('artisans').get('value');
	if (artisans =='') {
		alert('Please select at least one artisan.');
		window.parent.$j.colorbox.close();
	} else {
		var artisan = artisans.split(';');
		for (var i=0;i<artisan.length;++i){
					var x = new Request({
						url: '/scripts/jobcards/chart_data.php?action=repLabour&sdate='+sdate+'&edate='+edate+'&artisan='+artisan[i],
						method: 'get',
						noCache: true,
						async: false,
						onComplete: function(response){
							...
							if (data.Records == null) {
								...
								doc.text(data.Records[0].fullname,7,line);
								//doc.text('STEVENS, MR. MICHAEL',7,line);
								doc.setFontSize(10);
								line = line + 3;
								...
								doc.text('ASSIGNED',50,line);
								doc.text('COMPLETED',100,line);
								doc.text('HOURS',150,line);
								doc.text('DELAYS',190,line);
								doc.setFontStyle('normal');
								line = line + 5;
								for (var x=0;x<data.Records.length;++x){
									doc.text(data.Records[x].id,7,line);
									doc.text(data.Records[x].assigned_date,50,line);
									doc.text(data.Records[x].completion_date,100,line);
									doc.text(data.Records[x].hours,150,line);
									doc.text(data.Records[x].delays,190,line);
								}
		...
		doc.save('/scripts/jobcards/tmp/labour.pdf');
		window.parent.$j.colorbox.close();
	}
}
