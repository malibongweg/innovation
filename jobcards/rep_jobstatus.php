	...
	var x = new Request({
			...
			onComplete: function(response){
				...
				for (var i = 0; i<data.Records.length; ++i){
					var x = new Request({
						url: '/scripts/jobcards/chart_data.php?action=statusReportData&cmp='+data.Records[i].campus_code,
						method: 'get',
						noCahce: true,
						async: false,
						onComplete: function(response){
				...
				doc.save('/scripts/jobcards/tmp/test.pdf');
...
function drawChart(dbData,cde,cmp) {
	        ...
	        doc.addImage(imgData, "PNG", x, y, 100, 85);
			++cnt;
			if (cnt >= 5) {
				drawPageNo();
				++pageNo;
...
function setHeader() {
	...
	doc.setFontSize(10);
	doc.myText('Jobcards Ratings Report',{align: 'right'},1,15);
	doc.setFontSize(10);
