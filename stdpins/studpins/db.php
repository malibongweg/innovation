...
$sql2 = sprintf("select count(*) as cnt2, iadstno, iadidno, iadpassport from stud.iadbio where iadstno = '%s' and (iadidno='%s' or iadpassport='%s') group by iadstno, iadidno, iadpassport",$_POST['student_no'], $_POST['id_no'], $_POST['id_no']);
$result2 = oci_parse($con,$sql2);
oci_execute($result2);
$row2 = oci_fetch_object($result2);
$idno2 = $row2->IADSTNO;
$pass_no = $row2->IADPASSPORT;
...
$sql3 = sprintf("select count(*) as cnt3, iadstno, substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn as fullname from stud.iadbio t where iadstno = '%s' and substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn = '%s'
 group by iadstno,substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn",$_POST['student_no'], $_POST['f_name']);
...
$sql = sprintf("select count(*) as cnt7, t.iadstno, t.iadidno,t.iadpassport,
    substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn AS fullname, t.iadpasswd
     from stud.iadbio t where to_char(t.iaddate, 'YYYY')  =  to_char(sysdate, 'YYYY')
    and t.iadstno = '%s'
     and (t.iadidno = '%s' or t.iadpassport = '%s')
     and substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn = '%s'
     group by t.iadpasswd, t.iadstno,t.iadidno,t.iadpassport,substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn", $row1->IADSTNO, $row2->IADIDNO, $row2->IADPASSPORT, $row3->FULLNAME);
...
$fname = $_POST['f_name'];
    ...
    $sql = sprintf("select t.iadstno, t.iadidno,t.iadpassport,
    substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn AS fullname, t.iadpasswd
     from stud.iadbio t where to_char(t.iaddate, 'YYYY')  =  to_char(sysdate, 'YYYY')
    and t.iadstno = '%s'
     and t.iadidno = '%s'
     and substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn = '%s'", $_POST['student_no'], $_POST['id_no'], $_POST['f_name']);

    $result = oci_parse($con, $sql);
    oci_execute($result);
    $row = oci_fetch_object($result);

$sql1 = sprintf("select count(*) as cnt1 from stud.iadbio where iadstno = '%s'",$_POST['student_no']);
//echo $sql1;
$result1 = oci_parse($con,$sql1);
oci_execute($result1);
$row1 = oci_fetch_object($result1);
...
$sql2 = sprintf("select count(*) as cnt2 from stud.iadbio where iadstno = '%s' and (iadidno='%s' or iadpassport='%s')",$_POST['student_no'], $_POST['id_no'], $_POST['id_no']);
$result2 = oci_parse($con,$sql2);
oci_execute($result2);
$row2 = oci_fetch_object($result2);
$idno2 = $row2->IADSTNO;
...
$sql3 = sprintf("select count(*) as cnt3 from stud.iadbio t where iadstno = '%s' and substr(iadnames,1, instr(t.iadnames||' ', ' ')-1)||' '||t.iadsurn = '%s'",$_POST['student_no'], $_POST['f_name']);
$result3 = oci_parse($con,$sql3);
oci_execute($result3);
$row3 = oci_fetch_object($result3);
$fname = $row->FULLNAME;
