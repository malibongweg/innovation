...
if (isset($_GET)) {
	...
	$dbo->setQuery($sql);
       $result = $dbo->loadObjectList();
        ...
        $data = array();
	$sql = sprintf("select companyname, contactnumber, email from ups_register.company where companyid = %d", ($_GET['id']));  
	...
	$dbo->setQuery($sql);
       $result = $dbo->loadObjectList();
        ...
        foreach($result as $row) {
            echo $row->companyname.";".$row->contactnumber.";".$row->email;
        }
       // echo json_encode($data);
	}
