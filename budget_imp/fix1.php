...
$sql = "select upper(cc) as cc,acc,amt as amt from fix1";
$res = mysql_query($sql,$db);
while ($row = mysql_fetch_object($res)){
	
	$sql = sprintf("insert into miss.maebud (maeglcc,maeglacc,maednamt,maeapamt,maegldcc,maegldacc,maeprmset) values ('%s','%s',%0.0f,%0.0f,8008,83800,'CP16')",$row->cc,$row->acc,$row->amt,$row->amt);
